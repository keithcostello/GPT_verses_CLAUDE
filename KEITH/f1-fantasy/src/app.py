"""F1 Fantasy Suzuka Dashboard — Streamlit app."""
...
def team_color_bar(team: str) -> str:
    """Return HTML div with team color left border."""
...
def render_constructor_card(constructor: dict):
    """Render a constructor pick card."""
...
def render_projection_table(predictions: list):
    """Render ranked predictions table."""
    st.markdown("### 📊 Projected Rankings")
    for i, driver in enumerate(predictions[:10]):
        color = TEAM_COLORS.get(driver.get('team', ''), "#888888")
        bar = f"🎨"
        st.markdown(f"{bar} **{driver['name']}** ({driver.get('team', '')}) — ${driver['price']/1e6:.1f}M — **{driver['projected_ppm']:.2f} PPM**")
...
def main():
    ...
    <div style="background:#0d1117; padding: 16px; border-radius: 8px; margin-bottom: 16px;">
        <h1 style="color:#f0f6fc; margin:0;">🏎️ F1 Fantasy — Suzuka 2026</h1>
    ...
    with col1:
        st.metric("Budget", "$100M", label_visibility="collapsed")
    with col2:
        st.metric("Drivers", "5 / 5", label_visibility="collapsed")
    with col3:
        st.metric("Constructors", "2 / 2", label_visibility="collapsed")
    ...
    # Get driver and constructor lists for optimizer
    drivers = []
    ...
    try:
        ...
        with col_projs:
            render_projection_table(predictions)

    except ImportError as e:
        st.error(f"OR-tools not installed: {e}")
        st.info("Run: `pip install ortools`")
